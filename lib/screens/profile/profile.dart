...
Future<void> getImageUrl()async {
  WidgetsBinding.instance?.addPostFrameCallback((timeStamp) async{
    x = (await _firestore.collection('users').doc(user.uid).get())['imageUrl'];
    setState((){});
  });
  ...
  Widget build(BuildContext context) {
   ...
   // print('updatePhotoURL=${user.photoURL}');
    return Scaffold(
      backgroundColor: Colors.grey[200],
      drawer: Drawer(
        child: Container(
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  final z = await y.getImage(
                                    ...
                                    if (z != null) {
                                      x = z.path;
                                      final upload = await  storage.ref().child(user.uid+'/').putFile(File(x!)).then((p0) async{
                                      String url = await p0.ref.getDownloadURL();
                                      print('image url = $url');
                                      await _firestore.collection('users').doc(user.uid).set({'imageUrl':url},SetOptions(merge: true));
                                      x = url;
                                      });
                                      setState((){});
                                    }
          ...
          Padding(
            ...
            child: CircleAvatar(
              ...
              child: CircleAvatar(
                ...
                //      : null,
               // backgroundImage: AssetImage('assetName'),
                foregroundImage: x != null ? NetworkImage(x!):const AssetImage('assets/images/burger.png') as ImageProvider ,
              ),
              //(x==null)?Text("No Image"):Image.file(x),
            ),
          ),
